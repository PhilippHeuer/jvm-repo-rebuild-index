...
func processFiles(files []string) (map[string]*model.DependencyMetadata, map[string]*model.DependencyMetadata) {
	depMetadata := make(map[string]*model.DependencyMetadata)
	projectMetadata := make(map[string]*model.DependencyMetadata)
	...
	for _, mvnMetadataFile := range files {
		...
		go func(file string) {
			...
			for k, v := range data {
				depMetadata[k] = v
				slog.Debug("added to data", "key", k, "versions", len(v.Versions))
			}
...
func processFile(mvnMetadataFile string) (map[string]*model.DependencyMetadata, map[string]*model.DependencyMetadata, error) {
	result := make(map[string]*model.DependencyMetadata)        // individual artifact metadata
	projectResult := make(map[string]*model.DependencyMetadata) // project metadata
	...
	for _, buildInfoFile := range buildInfoFiles {
		...
		if _, ok := projectResult[projectKey]; !ok {
			projectResult[projectKey] = &model.DependencyMetadata{
...
func writeProjectIndexToFilesystem(outputDir string, data map[string]*model.DependencyMetadata) {
			...
			if data.Latest != "" {
				...
				if latestVersionFound {
					badge := model.Badge{
						...
						Message:       fmt.Sprintf("%s - %d module(s) [%d ok / %d error]", data.Latest, len(data.Modules), latestVersion.ReproducibleFiles, latestVersion.NonReproducibleFiles),
